...
- **Campaign-research placement (`CAMPAIGN_RESEARCH_PLACEMENT`)**: selects which model bucket the `campaign_researcher` sub-agent runs on, via `ResearchConfiguration.campaign_models()` in `creative_agent/config.py`. Default `regional_25` pins campaign research to `gemini-2.5-flash`/`-lite` @ `us-central1` (PR #101 — halves per-base-model contention in the one `ParallelAgent`, since trend research already occupies the `global` gemini-3.x bucket). Alternate arms (`global_3x`, `global_altbucket`) exist to quantify that spread via the quota-spread DoE — see `experiments/quota_spread/` and [experiments/README.md](experiments/README.md). Leave unset for production behavior.
